...
//PHP에서는 되도록이면 error_reporting() 함수를 이용하여 개발 후에는 단순한 경고메세지는 출력이 되지 않도록 해주십시요.
  ...
  $mertkey = $oper_info->pay_key; //데이콤에서 발급한 상점키로 변경해 주시기 바랍니다.
...
exit;
	if ($hashdata2 == $hashdata) {          //해쉬값 검증이 성공하면
		...
		if($respcode == "0000"|| $rescode == "C000"){            //결제가 성공이면
...
<input type=hidden name=rescode value="<?=$respcode?>">	<!-- 결과코드 0000 이외는 실패 -->
<input type=hidden name=resmsg value="<?=$respmsg?>">	<!-- 응답메세지  -->
<input type=hidden name=orderid value="<?=$oid?>">		<!-- 상점 주문번호 -->
