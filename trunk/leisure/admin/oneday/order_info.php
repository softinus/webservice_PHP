...
$sql = "select * from wiz_dayproduct a, wiz_dayorder b where a.prdcode = b.prdcode and orderid = '$orderid'";
...
// 쿠폰번호 생성
function setCouponNumber(id) {
	
	document.getElementById("coupon_number").value="_U"+id;
}
...
function onViewCoupon(){
	var url="/oneday/order_coupon.php?orderid=<?=$orderid?>";
	window.open(url,"COUPON","width=700,height=590,menubar=no, scrollbars=no, resizable=no, toolbar=no, status=no, left=50, top=50")
}

function onSendSMS(formObj){
	var couponNumber = formObj.coupon_number.value;
//	alert(formObj.tagName)

//	alert(formObj.coupon_number)

	var url = "coupon_sms.php?coupon_number="+couponNumber+"&selorder=<?=$orderid?>";
	window.open(url,"SMS","height=350, width=350, menubar=no, scrollbars=no, resizable=no, toolbar=no, status=no, top=100, left=100");

}
function onSendEmail(formObj){
}
-->
</script>
...
?>
	<tr bgcolor="#FFFFFF">
		...
		<td><a href='/index.php?prdidx=<?=$row->prdcode?>' target='_blank'><img src='<?=$row->prdimg?>' width='50' height='50' border='0'></a></td>
		<td><a href='/index.php?prdidx=<?=$row->prdcode?>' target='_blank'><?=$row->prdname?></a><?=$del_type?></td>
...
if($row->opttitle3 != '') echo "$row->opttitle3 : $arroptcode3[0] <br>";
if($row->opttitle4 != '') echo "$row->opttitle4 : $arroptcode4[0] <br>";
if($row->opttitle5 != '') echo "$row->opttitle5 : $arroptcode5[0] <br>";
if($row->opttitle6 != '') echo "$row->opttitle6 : $arroptcode6[0] <br>";
if($row->opttitle7 != '') echo "$row->opttitle7 : $arroptcode7[0]  <br>";

if($row->opttitle != '') echo $row->opttitle;
if($row->opttitle != '' && $row->opttitle2 != '') echo "/";
if($row->opttitle2 != '') echo $row->opttitle2;
if($row->opttitle != '' || $row->opttitle2 != '') echo " : ".$arroptcode[0]." <br>";
?>
		</td>
		...
		<td>
			<table width="100%" border="0" cellspacing="1" cellpadding="6" class="t_style">
				...
				<tr>
					...
					<td class="t_value" colspan="5"><textarea name="memo" class="input" style="width:100%;height:100px"></textarea></td>
				</tr>
<?
...
?>
				...
				<tr>
					...
					<td class="t_value"><input type="text" name="account" class="input"></td>
					<td class="t_name">예금주</td>
					<td class="t_value"><input type="text" name="acc_name" class="input"></td>
				</tr>
<?
}
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" height="38">
	...
	<tr>
		...
		<td align="right">
			상품합계( <b><font color="#ED1C24"><?=number_format($prd_price)?>원</font></b> )
			<?=$discount_msg?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="2">
	<tr>
		<td class="tit_sub"><img src="../image/ics_tit.gif"> 쿠폰발급</td>
	</tr>
</table>
<form name="frm" action="order_save.php" method="post" style="display:inline;">
	...
	<input type="hidden" name="isdeliver" value="<?=$order_info->isdeliver?>">
	<input type="hidden" name="orgstatus" value="<?=$order_info->status?>">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td>
			<table width="100%" border="0" cellspacing="1" cellpadding="6" class="t_style">
					...
					<td class="t_value" colspan="3">
						...
						<? } else { ?>
							<select name="chg_status">
								...
								?>
								<option value="OY" <? if($order_info->status == "OY") echo "selected"; ?>>결제완료</option>
								<? if($order_info->isdeliver=="Y") : ?>
								<option value="DR" <? if($order_info->status == "DR") echo "selected"; ?>>발급(배송)대기</option>
								<option value="DI" <? if($order_info->status == "DI") echo "selected"; ?>>발급(배송)처리</option>
								<option value="DC" <? if($order_info->status == "DC") echo "selected"; ?>>발급(배송)완료</option>								
								<? else : ?>
								<option value="DC" <? if($order_info->status == "DC") echo "selected"; ?>>발급(배송)완료</option>								
								<? endif; ?>								
						...
						<? } ?> * 쿠폰 상품일 경우 '발급완료' 상태로 변경시 쿠폰이 자동으로 발급됩니다.
					</td>
				</tr>
				<tr>
					...
					<td class="t_value" colspan="3">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr class="t_name">
								...
								<td width="25%" align="center">발급/발송완료</td>
				...
				<? if($order_info->isdeliver=="Y") : ?>
				<tr>
					<td width="15%" class="t_name">송장번호</td>
					<td width="35%" class="t_value"><input name="deliver_num" id="deliver_num" type="text" value="<?=$order_info->deliver_num?>" class="input">
					<td width="15%" class="t_name">발송일자</td>
					<td width="35%" class="t_value"><input name="deliver_date" type="text" value="<?=$order_info->deliver_date?>" class="input"><br /> * 입력형식(년월일시분)<br />
예) 2011년 01월 16일 16시 23분 = 201101161623</td>
				</tr>
				<? else : ?>				
				<tr>
					<td width="15%" class="t_name">쿠폰번호</td>
					<td width="35%" class="t_value"><input name="coupon_number" id="coupon_number" type="text" value="<?=$order_info->coupon_number?>" class="input">			<a href="javascript:onViewCoupon();">쿠폰 미리보기</a></td>
					<td width="15%" class="t_name">쿠폰발급일</td>
					<td width="35%" class="t_value"><input name="coupon_date" type="text" value="<?=$order_info->coupon_date?>" class="input"><img src="../image/ic_cal.gif" style="cursor:hand" align="center" onClick="Calendar1('document.frm','coupon_date');"></td>
				</tr>
				<!--
				<tr>
					<td align="center" class="t_value"colspan="4">--애니위즈
						<span onclick="onSendSMS(document.frm)">쿠폰SMS재발송</span>
						<span onclick="onSendMail(document.frm)">쿠폰메일재발송</span>
					</td>
				</tr>
				-->
				<? endif; ?>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="2">
			  <tr>
			    <td class="tit_sub"><img src="../image/ics_tit.gif"> 주문정보</td>
			  </tr>
			</table>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td>
            <table width="100%" border="0" cellspacing="1" cellpadding="6" class="t_style">
              <tr>
                ...
                <td width="35%" class="t_value"><?=pay_method($order_info->pay_method)?> (카드승인번호 : <?=$order_info->tno?>)</td>
              </tr>
              <tr>
          ...
          <td>
            <table width="100%" border="0" cellspacing="1" cellpadding="6" class="t_style">
              <tr>
                ...
                <td class="t_value"><input name="rece_name" type="text" value="<?=$order_info->rece_name?>" class="input"></td>
				<td width="15%" class="t_name">이메일</td>
                <td width="35%" class="t_value"><input name="rece_email" type="text" value="<?=$order_info->rece_email?>" class="input"> <a href="javascript:sendEmail('<?=$order_info->rece_name?>','<?=$order_info->rece_email?>')";><img src="../image/btn_send_s.gif" border="0" align="absmiddle"></a></td>
              </tr>
              <tr>
