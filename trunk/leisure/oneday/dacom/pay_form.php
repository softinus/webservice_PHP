<?
    ...
    $LGD_NOTEURL            = "http://".$_SERVER["HTTP_HOST"]."/oneday/dacom/order_update.php";          //상점결제결과 처리(DB) 페이지(URL을 변경해 주세요)
    $LGD_CASNOTEURL			= "http://".$_SERVER["HTTP_HOST"]."/oneday/dacom/order_update_vir.php";		//무통장일경우에 출력되는 페이지
...
function doPay_ActiveX(){
    ...
    if (ret=="00"){     //ActiveX 로딩 성공
        ...
        if( "0000" == LGD_RESPCODE ) { //결제성공
             ...
             * 결제성공 화면 처리
             */
             document.location.replace("/oneday/order_ok.php?orderid=" + LGD_OID + "&rescode=0000&pay_method=<?=$order_info->pay_method?>&resmsg=" + LGD_NOTEURL_RESULT);
        } else { //결제실패
