...
function setCoupon(per){
		...
		if(document.forms[i].id != null){
			if(document.forms[i].coupon_check){
				if(document.forms[i].coupon_check.checked){
					...
					coupon_dis =  document.forms[i].coupon_dis.value
				}
			}
		}
	}
	...
	if(per=="Y"){
		opener.frm.coupon_persent.value = coupon_dis
	}
	alert("적용되었습니다.");
}
//-->
...
//				$sql = "select prdcode, prdprice, amount from wiz_basket_tmp where uniq_id='".$_COOKIE["uniq_id"]."'";
				...
				while($row = mysql_fetch_array($result)) {
					...
					$basket_list[$no][prdprice] = $row[sellprice];
					$basket_list[$no][amount] = $amount;
			  ...
			  <input type="hidden" name="discount_price" value="<?=$discount_price?>">
				<tr>
					...
					<td align="center"><input type="checkbox" name="coupon_check" value="<?=$prd_exsist?>" onClick="couponCheck(this.form,'<?=$per?>')"></td>
				</tr>
			...
			<table border="0" width="96%" align="center">
				...
				while($row = mysql_fetch_object($result)){
					if($row->coupon_type == "%"){
						$discount_price = $totalPrice*($row->coupon_dis/100);
						$per = "Y";
					}else{
						$discount_price = $row->coupon_dis;
						$per = "Y";
			  ...
			  <input type="hidden" name="prd_exsist" value="<?=$prd_exsist?>">
			  <input type="hidden" name="discount_price" value="<?=$discount_price?>">
			  <input type="hidden" name="coupon_dis" value="<?=$row->coupon_dis?>">
				<tr>
					...
					<td align="center"><input type="checkbox" name="coupon_check" onClick="setCoupon('<?=$per?>');"></td>
				</tr>
				<tr><td colspan=10 height=1 bgcolor=#f0f0f0></td></tr>
				</form>
				<?
